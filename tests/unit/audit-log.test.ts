...
describe("audit-log: getAuditLogs (consulta)", () => {
  it("reenvia los filtros al repository sin modificarlos", async () => {
    mockAuditLogRepo.findMany.mockResolvedValueOnce({ logs: [], total: 0 });
  ...
  it("devuelve lo que el repository responda", async () => {
    const fakeResult = {
      logs: [{ id: 1, action: "CREATE", entity: "EVENT" }],
      total: 1,
    };
    mockAuditLogRepo.findMany.mockResolvedValueOnce(fakeResult);
...
describe("audit-log: getAuditLogsHandler (scoping multi-tenant, antes imposible de probar)", () => {
  ...
  it("COMPANY_ADMIN queda forzado a actorRole:COMPANY_ADMIN, incluso si manda otro actorRole", async () => {
    vi.mocked(requireRole).mockResolvedValueOnce({
      userId: 2,
      role: "COMPANY_ADMIN",
      companyId: 5,
    ...
    await getAuditLogsHandler({ actorRole: "SUPER_ADMIN" });

    expect(mockAuditLogRepo.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ companyId: 5, actorRole: "COMPANY_ADMIN" }),
    );
  });

  it("SUPER_ADMIN no queda forzado a ningun actorRole (ve acciones de cualquier rol)", async () => {
    vi.mocked(requireRole).mockResolvedValueOnce({
      userId: 1,
      role: "SUPER_ADMIN",
      companyId: null,
  ...
  it("paginacion: usa page/pageSize por defecto (1/25) y los devuelve en la respuesta", async () => {
    vi.mocked(requireRole).mockResolvedValueOnce({
      userId: 1,
      role: "SUPER_ADMIN",
      companyId: null,
    ...
    expect(result).toEqual({
      success: true,
      data: { logs: [{ id: 1 }], total: 30, page: 1, pageSize: 25 },
    });
  });

  it("paginacion: respeta page/pageSize explicitos", async () => {
    vi.mocked(requireRole).mockResolvedValueOnce({
      userId: 1,
      role: "SUPER_ADMIN",
      companyId: null,
    ...
    await getAuditLogsHandler({ page: 3, pageSize: 10 });
